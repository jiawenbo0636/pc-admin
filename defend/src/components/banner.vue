  ...
  <div class="banner">
    <div class="title">
      <p>
        ...
        <input @input="keyup($event)" ref="text" type="text" placeholder="请输入标题" v-model="keyword">
      </p>
      <h1>
        <span>
      ...
      <div class="lie">
        ...
        <dialog-bar v-model="send" type="danger" :title="title" content="您确定要删除吗？" :cancel="clickCancel()" @danger="del(id)"  @confirm="upper(id)"></dialog-bar>
          <li v-for="(list,index) in lists">
            ...
            <span class="handle">
              <span>
                <router-link :to="{name:'newbanner',query:{id:list.id}}">
                  <img src="../assets/img/edit.png" />
                </router-link>
              </span>
              <span>
                <router-link :to="{name:'bannerdetails',query:{id:list.id}}">
  ...
  export default {
    ...
    data() {
      return {
        ...
        send:false,
        state:true,
        imgUrl: require("../assets/img/pic_ban.png"),
        keyword:'',
        arr: [],
        shown: false,
        num: 4,
    ...
    methods: {
      ...
      upper(val) {
        Axios.put(`/sys/banner/whether?id=${val}`)
          .then(response => {
            // 请求成功
            /*alert("上架成功")*/
            this.reload()
      ...
      goDetail() {
        this.$router.push("bannerdetails")
      }
    }
  }
</script>
